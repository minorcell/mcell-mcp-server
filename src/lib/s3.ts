...
export async function uploadToS3(args: UploadToS3Input, deps: UploadToS3Deps = {}): Promise<S3UploadResult> {
  ...
  const putResult = await client.send(
    new PutObjectCommand({
      ...
      ContentType: args.content_type ?? (typeof detectedContentType === 'string' ? detectedContentType : undefined),
